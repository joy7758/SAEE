#!/usr/bin/env python3
"""Validate human-filled RBAC evidence input before builder use.
...
from saee_backend.services.production_auth_evidence import RBAC_KEYS
...
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "rbac_approval_input_validation.local.json"
REPORT_PATH = OUTPUT_DIR / "rbac_approval_input_validation.md"
DOC_PATH = ROOT / "phase_b_product/commercial_readiness/RBAC_APPROVAL_INPUT_VALIDATOR_V0_1.md"
GATE_PATH = ROOT / "docs/strategy/SAEE_RBAC_APPROVAL_INPUT_VALIDATOR_RECOMMENDATION_GATE.md"
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_RBAC_APPROVAL_INPUT_VALIDATOR: FAIL " + message)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_RBAC_APPROVAL_INPUT_VALIDATOR: FAIL invalid input {path}: {exc}"
        ) from exc
...
def evidence_review_flags(data: dict[str, Any]) -> dict[str, bool]:
    ...
    if not isinstance(review, dict):
        review = {}
    return {key: review.get(key) is True for key in RBAC_KEYS}


def missing_evidence_review(data: dict[str, Any]) -> list[str]:
    flags = evidence_review_flags(data)
    return [key for key in RBAC_KEYS if not flags[key]]
...
def missing_source_notes(data: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(notes, dict):
        notes = {}
    return [key for key in RBAC_KEYS if not nonempty_text(notes.get(key))]
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "target_evidence_keys": list(RBAC_KEYS),
        "generated_by": "scripts/saee_rbac_approval_input_validator.py",
        ...
        "required_review_key_count": len(RBAC_KEYS),
        "completed_review_key_count": len(RBAC_KEYS) - len(missing_review),
        ...
        "next_action": (
            ...
            "request; otherwise complete missing RBAC source notes or resolve "
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE RBAC Approval Input Validation
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE RBAC Approval Input Validator v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_RBAC_APPROVAL_INPUT_VALIDATOR: PASS "
