#!/usr/bin/env python3
"""Smoke check for the operations on-call rotation approval input validator."""
...
from scripts.saee_operations_on_call_rotation_approval_input_validator import (
    DEFAULT_OUTPUT_PATH,
    DOC_PATH,
    GATE_PATH,
    REPORT_PATH,
...
VALIDATOR_SCRIPT = (
    ROOT / "scripts/saee_operations_on_call_rotation_approval_input_validator.py"
)
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_OPERATIONS_ON_CALL_ROTATION_APPROVAL_INPUT_VALIDATOR_SMOKE: FAIL: "
            + message
        )
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    if unsafe:
        boundary_review["on_call_rotation_started_by_codex"] = True
    return {
        ...
        "decision_summary": "Fixture-only operations on-call validator smoke input.",
        "evidence_review": {key: True for key in ON_CALL_KEYS},
        "source_notes_by_key": {
            key: f"Fixture source note for {key}." for key in ON_CALL_KEYS
        },
...
def main() -> None:
    ...
    require(default_summary["missing_on_call_rotation_slots"], "default input must miss slots")
    ...
    entry = index.get("operations_on_call_rotation_approval_input_validator_v0_1", {})
    for key, value in {
        "status": "hold",
        "validator_type": "saee_operations_on_call_rotation_approval_input_validator",
        "target_blocker_id": "on_call_rotation",
    ...
    print(
        "SAEE_OPERATIONS_ON_CALL_ROTATION_APPROVAL_INPUT_VALIDATOR_SMOKE: PASS "
