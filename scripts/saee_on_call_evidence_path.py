#!/usr/bin/env python3
"""Prove the local on-call evidence path without starting on-call.

This path check uses temporary fixture-only on-call rotation evidence, converts
it through the existing on-call evidence builder, then feeds that evidence into
the support/SLA profile. It proves the wiring from human input to commercial
go/no-go without starting on-call rotation, publishing escalation schedules,
assigning incident commanders, contacting anyone, closing blockers by itself,
...
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "on_call_evidence_path.local.json"
REPORT_PATH = OUTPUT_DIR / "on_call_evidence_path_report.md"
DOC_PATH = ROOT / "phase_b_product/commercial_readiness/ON_CALL_EVIDENCE_PATH_V0_1.md"
GATE_PATH = ROOT / "docs/strategy/SAEE_ON_CALL_EVIDENCE_PATH_RECOMMENDATION_GATE.md"
...
def fixture_input() -> dict[str, Any]:
    return {
        ...
        "decision_summary": (
            "Fixture-only on-call path proof. This is not a real on-call "
            "rotation or incident operations approval."
        ),
...
def build_path(output_path: Path) -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        fixture_input_path = tmp / "on_call_input.fixture.json"
        builder_output_path = tmp / "on_call_builder_output.fixture.json"
        support_evidence_path = tmp / "production_support_sla_evidence.from_on_call.fixture.json"
        ...
        source_paths["on_call_rotation"] = support_evidence_path
    ...
    on_call_satisfied = "on_call_rotation" in set(
    ...
    result: dict[str, Any] = {
        ...
        "generated_by": "scripts/saee_on_call_evidence_path.py",
        ...
        "on_call_rotation_available_for_review": builder[
        ...
        "on_call_blocker_path_proven": on_call_satisfied,
        ...
        "next_action": (
            ...
            "and incident operations evidence, then rerun the on-call builder "
            "and support/SLA profile. This path proof alone closes no blockers."
        ),
    }
...
def write_report(result: dict[str, Any]) -> None:
    REPORT_PATH.write_text(
        "\n".join(
            [
                "# SAEE On-call Evidence Path Report v0.1",
                "",
                ...
                f"- on_call_rotation_available_for_review: {str(result['on_call_rotation_available_for_review']).lower()}",
                f"- on_call_blocker_path_proven: {str(result['on_call_blocker_path_proven']).lower()}",
                ...
                "",
                "This path proof uses fixture-only on-call evidence. It proves",
                "local evidence wiring, but it does not start an on-call rotation,",
                "publish an escalation schedule, assign an incident commander,",
                "contact customers or vendors, close blockers, launch product, or",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build SAEE on-call evidence path proof")
...
def main() -> None:
    ...
    if args.json:
        ...
        return
    print(
        "SAEE_ON_CALL_EVIDENCE_PATH: PASS "
        f"path_status={result['path_status']} "
        "fixture_only=true "
        f"on_call_blocker_path_proven={str(result['on_call_blocker_path_proven']).lower()} "
