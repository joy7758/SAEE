#!/usr/bin/env python3
"""Smoke check for the SAEE pricing-page evidence builder."""
...
INPUT_TEMPLATE = OUTPUT_DIR / "pricing_page_evidence_input.template.json"
BUILDER_OUTPUT = OUTPUT_DIR / "pricing_page_evidence_builder_output.local.json"
EVIDENCE_OUTPUT = OUTPUT_DIR / "production_billing_revenue_evidence.from_pricing_page.local.json"
REPORT = OUTPUT_DIR / "pricing_page_evidence_builder_report.md"
DOC = ROOT / "phase_b_product/commercial_readiness/PRICING_PAGE_EVIDENCE_BUILDER_V0_1.md"
GATE = ROOT / "docs/strategy/SAEE_PRICING_PAGE_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_PRICING_PAGE_EVIDENCE_BUILDER_SMOKE: FAIL: " + message)
...
def main() -> None:
    ...
    require(
        template.get("template_type") == "saee_pricing_page_evidence_input",
        "template type mismatch",
    )
    require(
        template.get("codex_published_pricing_page") is False,
        "template must not claim Codex published pricing page",
    )
    ...
    for token in [
        ...
        '"private_core_exposed": true',
        "pricing_page_available: true",
        "pricing_page_published: true",
        "pricing_page_approved: true",
        "public_price_points_approved: true",
        "customer_facing_pricing_page_created: true",
        "sales_offer_generated: true",
    ...
    from saee_pricing_page_evidence_builder import (
        TARGET_KEYS,
        build_from_input,
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        complete_input.update(
            {
                "human_reviewer_name": "human-pricing-page-reviewer",
                ...
                "review_record_reference": "internal-pricing-page-reference",
                "decision_summary": "Human pricing-page evidence supplied for smoke fixture.",
            }
        )
        ...
        require(
            complete_summary["pricing_page_evidence_complete_for_review"] is True,
            "complete fixture pricing-page evidence must be true",
        )
        require(
            complete_summary["production_billing_revenue_ready"] is False,
            "complete pricing-page fixture must not make all billing/revenue ready",
        )
        require(
        ...
        unsafe_input["pricing_page_published"] = True
        ...
        require(
            "pricing_page_published" in unsafe_summary["input_boundary_violations"],
            "unsafe fixture must report pricing_page_published",
        )

    llms = (ROOT / "llms.txt").read_text(encoding="utf-8")
    for path in [
        "/phase_b_product/commercial_readiness/PRICING_PAGE_EVIDENCE_BUILDER_V0_1.md",
        ...
        "/docs/strategy/SAEE_PRICING_PAGE_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md",
        "/scripts/saee_pricing_page_evidence_builder.py",
        "/scripts/saee_pricing_page_evidence_builder_smoke.py",
    ]:
        require(path in llms, f"llms.txt missing {path}")

    index = read_json(ROOT / "agent-index.json")
    entry = index.get("pricing_page_evidence_builder_v0_1", {})
    for key, expected in {
        "status": "local_builder_available_default_hold",
        "builder_scope": "human_filled_pricing_page_to_production_billing_revenue_evidence",
        "target_blocker": "pricing_page",
        "human_review_required": True,
        "pricing_page_evidence_complete_for_review": False,
        "production_billing_revenue_ready": False,
        "production_ready": False,
    ...
    print(
        "SAEE_PRICING_PAGE_EVIDENCE_BUILDER_SMOKE: PASS "
