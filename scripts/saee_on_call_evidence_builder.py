#!/usr/bin/env python3
"""Build on-call evidence from a human-filled operations input.

This builder converts local, human-filled on-call rotation evidence into the
production support/SLA evidence shape consumed by commercial readiness checks.
It does not start an on-call rotation, publish an escalation schedule, assign
an incident commander, contact customers or vendors, close blockers, modify
...
DEFAULT_INPUT_PATH = OUTPUT_DIR / "on_call_evidence_input.template.json"
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "on_call_evidence_builder_output.local.json"
DEFAULT_SUPPORT_OUTPUT_PATH = OUTPUT_DIR / "production_support_sla_evidence.from_on_call.local.json"
REPORT_PATH = OUTPUT_DIR / "on_call_evidence_builder_report.md"
DOC_PATH = ROOT / "phase_b_product/commercial_readiness/ON_CALL_EVIDENCE_BUILDER_V0_1.md"
GATE_PATH = ROOT / "docs/strategy/SAEE_ON_CALL_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md"
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_ON_CALL_EVIDENCE_BUILDER: FAIL: " + message)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_ON_CALL_EVIDENCE_BUILDER: FAIL: invalid input {path}: {exc}"
        ) from exc
...
def completed_on_call_slots(data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    slots = data.get("on_call_evidence_slots", [])
...
def build_support_evidence(
    ...
    *,
    ...
    evidence: dict[str, Any] = {
        ...
        "evidence_scope": "human_filled_on_call_rotation_to_production_support_evidence",
        "evidence_version": "v0.1",
        "generated_by": "scripts/saee_on_call_evidence_builder.py",
        ...
        "on_call_owner_recorded": bool(str(data.get("on_call_owner", "")).strip()),
        "incident_operations_owner_recorded": bool(
            str(data.get("incident_operations_owner", "")).strip()
        ),
        "completed_on_call_slot_count": len(complete_slots),
        "on_call_rotation_started_by_codex": False,
        "escalation_schedule_published_by_codex": False,
        "incident_commander_assigned_by_codex": False,
        "support_operations_started": False,
    ...
    for key in SUPPORT_CONTACT_KEYS + CUSTOMER_SUPPORT_KEYS + SLA_KEYS:
        evidence[key] = False
    for key in ON_CALL_KEYS:
...
def build_from_input(
    ...
    *,
    ...
    missing_slots = [key for key in ON_CALL_KEYS if key not in completed_on_call_slots(data)]
    ...
    summary: dict[str, Any] = {
        ...
        "missing_on_call_slot_count": len(missing_slots),
        "missing_on_call_slots": missing_slots,
        ...
        "on_call_rotation_available_for_review": readiness["on_call_rotation_available"],
        ...
        "codex_inferred_missing_evidence": False,
        "next_action": (
            "If a human fills the on-call evidence input with source notes, the "
            "generated support evidence can be used as one input to support "
            "readiness review. Support contact, customer support, and SLA "
...
def report_markdown(summary: dict[str, Any]) -> str:
    return f"""# SAEE On-call Evidence Builder Report
...
- on_call_rotation_available_for_review: {str(summary['on_call_rotation_available_for_review']).lower()}
- production_support_available: {str(summary['production_support_available']).lower()}
- blockers_closed_by_builder: 0

## What This Adds

This builder gives human reviewers a concrete way to convert a filled on-call
rotation input into the existing production support/SLA evidence shape. It
only targets the `on_call_rotation` evidence group.

## What It Does Not Do

It does not start an on-call rotation, publish an escalation schedule, assign
an incident commander, contact customers, contact support vendors, start
support operations, close blockers, or mark SAEE as production ready.
...
Human owners must fill `on_call_evidence_input.template.json` with real source
...
def write_docs(summary: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE On-call Evidence Builder v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_ON_CALL_EVIDENCE_BUILDER: PASS "
            f"path={args.output} status={summary['status']} "
            "on_call_rotation_available_for_review="
            f"{str(summary['on_call_rotation_available_for_review']).lower()} "
