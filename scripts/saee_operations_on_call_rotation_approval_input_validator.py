#!/usr/bin/env python3
"""Validate human-filled operations-on-call-rotation input before builder use.

This validator checks completeness and boundary safety only. It does not start
an on-call rotation, publish escalation schedules, assign incident commanders,
contact vendors/customers, run the evidence builder, close blockers, or claim
...
from saee_backend.services.production_operations_evidence import ON_CALL_KEYS
from scripts.saee_operations_on_call_rotation_evidence_builder import (
    DEFAULT_INPUT_PATH,
    INPUT_FORBIDDEN_TRUE_KEYS,
    boundary_violations,
    completed_on_call_rotation_slots,
...
DEFAULT_OUTPUT_PATH = (
    OUTPUT_DIR / "operations_on_call_rotation_approval_input_validation.local.json"
)
REPORT_PATH = OUTPUT_DIR / "operations_on_call_rotation_approval_input_validation.md"
DOC_PATH = (
    ROOT
    / "phase_b_product/commercial_readiness/"
    "OPERATIONS_ON_CALL_ROTATION_APPROVAL_INPUT_VALIDATOR_V0_1.md"
)
GATE_PATH = (
    ROOT
    / "docs/strategy/"
    "SAEE_OPERATIONS_ON_CALL_ROTATION_APPROVAL_INPUT_VALIDATOR_RECOMMENDATION_GATE.md"
)
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_OPERATIONS_ON_CALL_ROTATION_APPROVAL_INPUT_VALIDATOR: FAIL "
            + message
        )
...
def missing_on_call_rotation_slots(data: dict[str, Any]) -> list[str]:
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    slots_missing = missing_on_call_rotation_slots(data)
    ...
    on_call_rotation_slots_complete = not slots_missing
    input_complete = (
        ...
        and on_call_rotation_slots_complete
        and not violations
    )
    ...
    return {
        ...
        "validation_scope": (
            "local_human_filled_operations_on_call_rotation_input_pre_builder_check"
        ),
        "target_blocker_id": "on_call_rotation",
        "generated_by": "scripts/saee_operations_on_call_rotation_approval_input_validator.py",
        ...
        "on_call_rotation_slots_complete": on_call_rotation_slots_complete,
        "required_on_call_rotation_evidence_item_count": len(ON_CALL_KEYS),
        ...
        "missing_on_call_rotation_slots": slots_missing,
        ...
        "next_action": (
            "If validation_status is pass, a human may run the operations "
            "on-call rotation evidence builder in a separate approved evidence "
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE Operations On-call Rotation Approval Input Validation
...
{list_lines(validation['missing_on_call_rotation_slots'])}
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Operations On-call Rotation Approval Input Validator v0.1
...
target_blocker_id: on_call_rotation
required_on_call_rotation_evidence_item_count: {validation['required_on_call_rotation_evidence_item_count']}
blockers_closed_by_validator: 0
on_call_rotation_approved_by_validator: false
on_call_rotation_started_by_validator: false
escalation_schedule_published_by_validator: false
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_OPERATIONS_ON_CALL_ROTATION_APPROVAL_INPUT_VALIDATOR: PASS "
