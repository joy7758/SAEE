#!/usr/bin/env python3
"""Build production-monitoring evidence from a human-filled input.

This builder converts local, human-filled production-monitoring review
evidence into the production operations evidence shape consumed by commercial
readiness checks. It does not deploy monitoring, configure dashboards, contact
vendors, send alerts, modify product behavior, close blockers, or claim
...
DEFAULT_INPUT_PATH = OUTPUT_DIR / "production_monitoring_evidence_input.template.json"
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "production_monitoring_evidence_builder_output.local.json"
DEFAULT_OPERATIONS_OUTPUT_PATH = (
    OUTPUT_DIR / "production_operations_evidence.from_production_monitoring.local.json"
)
REPORT_PATH = OUTPUT_DIR / "production_monitoring_evidence_builder_report.md"
DOC_PATH = ROOT / "phase_b_product/commercial_readiness/PRODUCTION_MONITORING_EVIDENCE_BUILDER_V0_1.md"
GATE_PATH = (
    ROOT / "docs/strategy/SAEE_PRODUCTION_MONITORING_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md"
)
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_PRODUCTION_MONITORING_EVIDENCE_BUILDER: FAIL: " + message)
...
def default_input_template() -> dict[str, Any]:
    return {
        ...
        "decision_summary": "",
        "evidence_review": {key: False for key in PRODUCTION_MONITORING_KEYS},
        "source_notes_by_key": {key: "" for key in PRODUCTION_MONITORING_KEYS},
    ...
    if path.exists():
        if path.resolve() == DEFAULT_INPUT_PATH.resolve():
            ...
            if (
                ...
                or data.get("input_status") != "template_pending_human_input"
            ):
                write_json(path, default_input_template())
        return
    if not path.exists():
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_PRODUCTION_MONITORING_EVIDENCE_BUILDER: FAIL: invalid input {path}: {exc}"
        ) from exc
...
def evidence_review_flags(data: dict[str, Any]) -> dict[str, bool]:
    ...
    if not isinstance(review, dict):
        review = {}
    return {key: bool_value(review, key) for key in PRODUCTION_MONITORING_KEYS}
...
def source_notes(data: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(notes, dict):
        return {}
    return {key: str(notes.get(key, "")).strip() for key in PRODUCTION_MONITORING_KEYS}
...
def completed_monitoring_slots(data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    slots = data.get("monitoring_evidence_slots", [])
    ...
    for slot in slots:
        ...
        if key not in PRODUCTION_MONITORING_KEYS:
            continue
        if (
...
def complete_input(data: dict[str, Any]) -> bool:
    ...
    complete_slots = completed_monitoring_slots(data)
    return (
        data.get("template_type") == "saee_production_monitoring_evidence_input"
        and input_metadata_complete(data)
        and all(flags.values())
        and all(bool(notes.get(key)) for key in PRODUCTION_MONITORING_KEYS)
        and all(key in complete_slots for key in PRODUCTION_MONITORING_KEYS)
...
def build_operations_evidence(
    ...
    *,
    ...
    evidence: dict[str, Any] = {
        ...
        "evidence_scope": "human_filled_production_monitoring_to_production_operations_evidence",
        "evidence_version": "v0.1",
        "generated_by": "scripts/saee_production_monitoring_evidence_builder.py",
        ...
        "monitoring_owner_recorded": bool(str(data.get("monitoring_owner", "")).strip()),
        "operations_reviewer_name_recorded": bool(
            str(data.get("operations_reviewer_name", "")).strip()
        ),
        "completed_monitoring_slot_count": len(complete_slots),
        "monitoring_deployed_by_codex": False,
        "dashboard_configured_by_codex": False,
        "metrics_export_enabled_by_codex": False,
        "log_retention_changed_by_codex": False,
    ...
    for key in EXTERNAL_ALERT_DELIVERY_KEYS + ON_CALL_KEYS:
        evidence[key] = False
...
def build_from_input(
    ...
    *,
    ...
    flags = evidence_review_flags(data)
    missing = [key for key in PRODUCTION_MONITORING_KEYS if not flags[key]]
    missing_slots = [
        key for key in PRODUCTION_MONITORING_KEYS if key not in completed_monitoring_slots(data)
    ]
    ...
    summary: dict[str, Any] = {
        ...
        "completed_monitoring_slot_count": len(completed_monitoring_slots(data)),
        "required_evidence_item_count": len(PRODUCTION_MONITORING_KEYS),
        "provided_evidence_item_count": sum(1 for value in flags.values() if value),
        "missing_required_evidence_count": len(missing),
        "missing_required_evidence": missing,
        "missing_monitoring_slot_count": len(missing_slots),
        "missing_monitoring_slots": missing_slots,
        ...
        "production_monitoring_available_for_review": readiness["production_monitoring_available"],
        ...
        "target_blocker_ids": ["production_monitoring"],
        ...
        "codex_inferred_missing_evidence": False,
        "next_action": (
            "If a human fills the production-monitoring evidence input with "
            "source notes, the generated operations evidence can be used as "
            "one input to operations readiness review. External alert delivery "
...
def report_markdown(summary: dict[str, Any]) -> str:
    return f"""# SAEE Production Monitoring Evidence Builder Report
...
- production_monitoring_available_for_review: {str(summary['production_monitoring_available_for_review']).lower()}
...
def write_docs(summary: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Production Monitoring Evidence Builder v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_PRODUCTION_MONITORING_EVIDENCE_BUILDER: PASS "
            f"path={args.output} status={summary['status']} "
            "production_monitoring_available_for_review="
            f"{str(summary['production_monitoring_available_for_review']).lower()} "
