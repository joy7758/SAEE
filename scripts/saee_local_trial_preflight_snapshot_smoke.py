#!/usr/bin/env python3
"""Smoke check for the SAEE local trial preflight snapshot."""
...
SNAPSHOT_JSON = ROOT / "phase_b_product/validation/local_trial_preflight_snapshot.local.json"
SNAPSHOT_MD = ROOT / "phase_b_product/validation/local_trial_preflight_snapshot.md"
GATE = ROOT / "docs/strategy/SAEE_LOCAL_TRIAL_PREFLIGHT_SNAPSHOT_RECOMMENDATION_GATE.md"
RUNNER = ROOT / "scripts/saee_local_trial_preflight_snapshot.py"


def fail(message: str) -> None:
    raise SystemExit(f"SAEE_LOCAL_TRIAL_PREFLIGHT_SNAPSHOT_SMOKE: FAIL: {message}")
...
def main() -> None:
    ...
    require(payload.get("local_trial_preflight_snapshot_v0_1") is True, "snapshot flag")
    require(payload.get("snapshot_type") == "local_trial_preflight_snapshot", "snapshot type")
    require(payload.get("status") in {"pass", "hold"}, "status must be pass or hold")
    require(isinstance(payload.get("ready_to_start"), bool), "ready_to_start boolean")
    require(payload.get("preflight_scope") == "local_controlled_trial_demo_operator_check", "scope")
    require(payload.get("local_loopback_probe_only") is True, "local loopback probe only")
    require(payload.get("human_review_required") is True, "human review required")
    require(payload.get("blockers_closed_by_snapshot") == 0, "blockers closed")
    require(payload.get("prefers_local_venv_python") is True, "prefers local venv python")
    require(payload.get("selected_python_source") == "local_venv", "selected python source")
    ...
    print(
        "SAEE_LOCAL_TRIAL_PREFLIGHT_SNAPSHOT_SMOKE: PASS "
