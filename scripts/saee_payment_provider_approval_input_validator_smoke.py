#!/usr/bin/env python3
"""Smoke check for the payment provider approval input validator."""
...
from scripts.saee_payment_provider_evidence_builder import INPUT_FORBIDDEN_TRUE_KEYS, TARGET_KEYS
from scripts.saee_payment_provider_approval_input_validator import (
    DEFAULT_OUTPUT_PATH,
    DOC_PATH,
    GATE_PATH,
    REPORT_PATH,
...
VALIDATOR_SCRIPT = ROOT / "scripts/saee_payment_provider_approval_input_validator.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_PAYMENT_PROVIDER_APPROVAL_INPUT_VALIDATOR_SMOKE: FAIL: " + message
        )
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    return {
        ...
        "review_record_reference": "fixture://payment-provider/review-record",
        ...
        "review_artifacts": [
            {
                "evidence_key": key,
                "artifact_reference": f"fixture://payment-provider/{key}",
...
def main() -> None:
    ...
    entry = index.get("payment_provider_approval_input_validator_v0_1", {})
    for key, value in {
        "status": "hold",
        "validator_type": "saee_payment_provider_approval_input_validator",
        "target_blocker_id": "payment_provider",
    ...
    print(
        "SAEE_PAYMENT_PROVIDER_APPROVAL_INPUT_VALIDATOR_SMOKE: PASS "
