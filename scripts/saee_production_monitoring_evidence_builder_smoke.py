#!/usr/bin/env python3
"""Smoke check for the SAEE production-monitoring evidence builder."""
...
BUILDER_SCRIPT = ROOT / "scripts/saee_production_monitoring_evidence_builder.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_PRODUCTION_MONITORING_EVIDENCE_BUILDER_SMOKE: FAIL: " + message)
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    source_notes = {
        key: f"Human-reviewed production-monitoring source note for {key}."
    ...
    return {
        ...
        "decision_summary": "Fixture-only monitoring evidence for deterministic smoke validation.",
...
def main() -> None:
    ...
    require(
        default_summary["production_monitoring_available_for_review"] is False,
        "default production monitoring must not be available",
    )
    require(
    ...
    require(
        complete_summary["production_monitoring_available_for_review"] is True,
        "complete fixture production monitoring available",
    )
    require(
    ...
    require(
        complete_readiness["production_monitoring_available"] is True,
        "complete readiness production monitoring available",
    )
    ...
    require(
        unsafe_readiness["production_monitoring_available"] is False,
        "unsafe monitoring remains unavailable",
    )
    ...
    for path in [
        "/phase_b_product/commercial_readiness/PRODUCTION_MONITORING_EVIDENCE_BUILDER_V0_1.md",
        "/docs/strategy/SAEE_PRODUCTION_MONITORING_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md",
        "/phase_b_product/commercial_readiness/operations_evidence/production_monitoring_evidence_input.template.json",
        "/phase_b_product/commercial_readiness/operations_evidence/production_monitoring_evidence_builder_output.local.json",
        "/phase_b_product/commercial_readiness/operations_evidence/production_operations_evidence.from_production_monitoring.local.json",
        "/phase_b_product/commercial_readiness/operations_evidence/production_monitoring_evidence_builder_report.md",
        "/scripts/saee_production_monitoring_evidence_builder.py",
        "/scripts/saee_production_monitoring_evidence_builder_smoke.py",
    ]:
        require(path in llms, "llms.txt missing path: " + path)

    index = json.loads((ROOT / "agent-index.json").read_text(encoding="utf-8"))
    entry = index.get("production_monitoring_evidence_builder_v0_1", {})
    expected = {
    ...
    print(
        "SAEE_PRODUCTION_MONITORING_EVIDENCE_BUILDER_SMOKE: PASS "
        "default_hold=true complete_fixture_monitoring_available=true "
