#!/usr/bin/env python3
"""Validate human-filled payment-provider input before the evidence builder.

This validator checks completeness and boundary safety only. It does not
select or contact a payment provider, configure test or live mode, enable
checkout, create payment links, configure webhooks, collect payment, validate
revenue, close blockers, or claim production readiness.
"""
...
from scripts.saee_payment_provider_evidence_builder import (
    DEFAULT_INPUT_PATH,
...
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "payment_provider_approval_input_validation.local.json"
REPORT_PATH = OUTPUT_DIR / "payment_provider_approval_input_validation.md"
DOC_PATH = (
    ROOT
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_PAYMENT_PROVIDER_APPROVAL_INPUT_VALIDATOR: FAIL " + message)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            "SAEE_PAYMENT_PROVIDER_APPROVAL_INPUT_VALIDATOR: FAIL "
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_by": "scripts/saee_payment_provider_approval_input_validator.py",
        ...
        "required_payment_provider_evidence_item_count": len(TARGET_KEYS),
        ...
        "next_action": (
            "If validation_status is pass, a human may run the payment provider "
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE Payment Provider Approval Input Validation
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Payment Provider Approval Input Validator v0.1
...
target_blocker_id: payment_provider
required_payment_provider_evidence_item_count: {validation['required_payment_provider_evidence_item_count']}
blockers_closed_by_validator: 0
payment_provider_approved_by_validator: false
payment_provider_selected_by_validator: false
payment_provider_configured_by_validator: false
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_PAYMENT_PROVIDER_APPROVAL_INPUT_VALIDATOR: PASS "
