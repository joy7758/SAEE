#!/usr/bin/env python3
"""Build privacy/legal and DPA evidence from human-filled review input.

This builder converts local, human-filled privacy/legal and DPA review input
into the production privacy/security/legal evidence shape consumed by
commercial readiness checks. It does not perform legal review, contact legal
counsel, create or approve a DPA, process customer data, close blockers,
...
DEFAULT_INPUT_PATH = OUTPUT_DIR / "privacy_legal_dpa_evidence_input.template.json"
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "privacy_legal_dpa_evidence_builder_output.local.json"
...
REPORT_PATH = OUTPUT_DIR / "privacy_legal_dpa_evidence_builder_report.md"
DOC_PATH = (
    ROOT / "phase_b_product/commercial_readiness/PRIVACY_LEGAL_DPA_EVIDENCE_BUILDER_V0_1.md"
)
GATE_PATH = (
    ROOT / "docs/strategy/SAEE_PRIVACY_LEGAL_DPA_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md"
)

TARGET_KEYS = PRIVACY_LEGAL_REVIEW_KEYS + DPA_KEYS
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_PRIVACY_LEGAL_DPA_EVIDENCE_BUILDER: FAIL: " + message
        )
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_PRIVACY_LEGAL_DPA_EVIDENCE_BUILDER: FAIL: invalid input {path}: {exc}"
        ) from exc
...
def build_privacy_security_legal_evidence(
    ...
    *,
    ...
    evidence: dict[str, Any] = {
        ...
        "evidence_scope": "human_filled_privacy_legal_dpa_review_to_production_privacy_security_legal_evidence",
        "evidence_version": "v0.1",
        "generated_by": "scripts/saee_privacy_legal_dpa_evidence_builder.py",
        ...
        "privacy_owner_recorded": bool(str(data.get("privacy_owner", "")).strip()),
        "dpa_owner_recorded": bool(str(data.get("dpa_owner", "")).strip()),
        "review_record_reference_recorded": bool(str(data.get("review_record_reference", "")).strip()),
    ...
    for key in FORMAL_SECURITY_REVIEW_KEYS + VULNERABILITY_MANAGEMENT_KEYS:
        evidence[key] = False
    for key in PRIVACY_LEGAL_REVIEW_KEYS + DPA_KEYS:
        evidence[key] = flags[key] and complete
    for key in FORBIDDEN_TRUE_KEYS:
        evidence[key] = False
    evidence["privacy_legal_review_completed_by_codex"] = False
    evidence["data_processing_agreement_completed_by_codex"] = False
    evidence["legal_review_claim_published"] = False
    evidence["dpa_availability_claim_published"] = False
    evidence["customer_data_processing_claim_published"] = False
    return evidence
...
def build_from_input(
    ...
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "privacy_legal_review_completed_for_review": readiness_result[
            "privacy_legal_review_completed"
        ],
        "data_processing_agreement_available_for_review": readiness_result[
        ...
        "legal_review_execution_authorized": False,
        "next_action": (
            "Human legal and privacy owners may fill the privacy/legal and DPA "
            "input with source-backed approval references. The generated "
            "evidence remains one input to a later go/no-go profile."
        ),
    }
...
def report_markdown(summary: dict[str, Any]) -> str:
    return f"""# SAEE Privacy/Legal + DPA Evidence Builder Report
...
- privacy_legal_review_completed_for_review: {str(summary['privacy_legal_review_completed_for_review']).lower()}
- data_processing_agreement_available_for_review: {str(summary['data_processing_agreement_available_for_review']).lower()}
...
def write_docs(summary: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Privacy/Legal + DPA Evidence Builder v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_PRIVACY_LEGAL_DPA_EVIDENCE_BUILDER: PASS "
