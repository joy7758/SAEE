#!/usr/bin/env python3
"""Smoke check for the SLA approval input validator."""
...
from scripts.saee_sla_evidence_builder import INPUT_FORBIDDEN_TRUE_KEYS
from scripts.saee_sla_approval_input_validator import (
    DEFAULT_OUTPUT_PATH,
    DOC_PATH,
    GATE_PATH,
    REPORT_PATH,
...
VALIDATOR_SCRIPT = ROOT / "scripts/saee_sla_approval_input_validator.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_SLA_APPROVAL_INPUT_VALIDATOR_SMOKE: FAIL: " + message)
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    return {
        ...
        "decision_summary": "Fixture-only SLA validator smoke input.",
        "evidence_review": {key: True for key in SLA_KEYS},
        "source_notes_by_key": {
            key: f"Fixture source note for {key}." for key in SLA_KEYS
        },
...
def main() -> None:
    ...
    require(default_summary["missing_sla_slots"], "default must miss SLA slots")
    ...
    require(
        complete_summary["sla_available_by_validator"] is False,
        "validator does not make SLA available",
    )
    ...
    entry = index.get("sla_approval_input_validator_v0_1", {})
    for key, value in {
        "status": "hold",
        "validator_type": "saee_sla_approval_input_validator",
        "target_blocker_id": "sla",
    ...
    print(
        "SAEE_SLA_APPROVAL_INPUT_VALIDATOR_SMOKE: PASS "
