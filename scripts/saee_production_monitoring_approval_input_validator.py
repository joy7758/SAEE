#!/usr/bin/env python3
"""Validate human-filled production-monitoring input before the evidence builder.

This validator checks completeness and boundary safety only. It does not deploy
...
DEFAULT_HUMAN_FILLED_INPUT_PATH = (
    OUTPUT_DIR / "production_monitoring_evidence_input.human_filled.local.json"
)
DEFAULT_OUTPUT_PATH = (
    OUTPUT_DIR / "production_monitoring_approval_input_validation.local.json"
)
REPORT_PATH = OUTPUT_DIR / "production_monitoring_approval_input_validation.md"
DOC_PATH = (
    ROOT
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_PRODUCTION_MONITORING_APPROVAL_INPUT_VALIDATOR: FAIL " + message
        )
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            "SAEE_PRODUCTION_MONITORING_APPROVAL_INPUT_VALIDATOR: FAIL "
...
def missing_evidence_review(data: dict[str, Any]) -> list[str]:
    ...
    return [key for key in PRODUCTION_MONITORING_KEYS if not flags[key]]


def missing_source_notes(data: dict[str, Any]) -> list[str]:
    notes = source_notes(data)
    return [key for key in PRODUCTION_MONITORING_KEYS if not notes.get(key)]


def missing_monitoring_slots(data: dict[str, Any]) -> list[str]:
    complete_slots = completed_monitoring_slots(data)
    return [key for key in PRODUCTION_MONITORING_KEYS if key not in complete_slots]
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    slots_missing = missing_monitoring_slots(data)
    ...
    monitoring_slots_complete = not slots_missing
    input_complete = (
    ...
    return {
        ...
        "generated_by": "scripts/saee_production_monitoring_approval_input_validator.py",
        ...
        "monitoring_slots_complete": monitoring_slots_complete,
        "required_monitoring_evidence_item_count": len(PRODUCTION_MONITORING_KEYS),
        ...
        "missing_monitoring_slots": slots_missing,
        ...
        "next_action": (
            "If validation_status is pass, a human may run the production monitoring "
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE Production Monitoring Approval Input Validation
...
{list_lines(validation['missing_monitoring_slots'])}
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Production Monitoring Approval Input Validator v0.1
...
target_blocker_id: production_monitoring
required_monitoring_evidence_item_count: {validation['required_monitoring_evidence_item_count']}
blockers_closed_by_validator: 0
production_monitoring_approved_by_validator: false
production_monitoring_deployed_by_validator: false
dashboard_configured_by_validator: false
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_PRODUCTION_MONITORING_APPROVAL_INPUT_VALIDATOR: PASS "
