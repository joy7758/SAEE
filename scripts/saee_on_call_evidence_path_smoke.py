#!/usr/bin/env python3
"""Smoke check for the SAEE on-call evidence path proof."""
...
from scripts.saee_on_call_evidence_path import (
    DEFAULT_OUTPUT_PATH,
    DOC_PATH,
    GATE_PATH,
    REPORT_PATH,
...
RUNNER = ROOT / "scripts/saee_on_call_evidence_path.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_ON_CALL_EVIDENCE_PATH_SMOKE: FAIL: " + message)
...
def main() -> None:
    ...
    require(result["on_call_evidence_path_v0_1"] is True, "path flag true")
    require(
        result["path_type"] == "local_fixture_only_on_call_evidence_path",
        "path type changed",
    )
    ...
    require(
        result["real_incident_commander_assigned"] is False,
    ...
    require(
        result["on_call_rotation_available_for_review"] is True,
        "on-call available in fixture",
    )
    require(result["on_call_blocker_path_proven"] is True, "on-call path proven")
    require(
        result["support_profile_target_blockers_satisfied"] == ["on_call_rotation"],
        "only on_call_rotation should be satisfied",
    )
    require(
    ...
    entry = index.get("on_call_evidence_path_v0_1", {})
    expected = {
    ...
    print(
        "SAEE_ON_CALL_EVIDENCE_PATH_SMOKE: PASS "
        "fixture_only=true on_call_path_proven=true "
