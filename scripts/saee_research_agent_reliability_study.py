#!/usr/bin/env python3
"""Run and persist the fixed 3 x 5 research-agent reliability study."""
...
SCHEMA = ROOT / "agent-interface/reliability/saee-research-agent-reliability-study.schema.v0.2.json"
RESULT = ROOT / "agent-interface/reliability/saee-research-agent-reliability-result.v0.2.json"
REPORT = ROOT / "docs/research/SAEE_RESEARCH_AGENT_RELIABILITY_STUDY_V0_2.md"


def progress(agent: str, run_number: int, status: str) -> None:
    print(f"research_reliability_run agent={agent} run={run_number}/5 status={status}", flush=True)


def main() -> int:
    study = run_research_reliability_suite(live_ark_clients(), runs=5, progress=progress)
    schema = json.loads(SCHEMA.read_text(encoding="utf-8"))
    errors = list(Draft202012Validator(schema).iter_errors(study))
    if errors:
        raise ValueError(f"research reliability schema invalid: {errors[0].message}")
    RESULT.write_text(json.dumps(study, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    REPORT.write_text(build_research_report(study), encoding="utf-8")
    print("SAEE_RESEARCH_AGENT_RELIABILITY_STUDY_RESULT")
