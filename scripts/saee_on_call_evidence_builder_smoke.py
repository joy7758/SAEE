#!/usr/bin/env python3
"""Smoke check for the SAEE on-call evidence builder."""
...
BUILDER_SCRIPT = ROOT / "scripts/saee_on_call_evidence_builder.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_ON_CALL_EVIDENCE_BUILDER_SMOKE: FAIL: " + message)
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    source_notes = {
        key: f"Human-reviewed on-call source note for {key}." for key in ON_CALL_KEYS
    }
    ...
    return {
        ...
        "on_call_owner": "Fixture Operations Owner",
        "incident_operations_owner": "Fixture Incident Operations Owner",
        "decision_summary": "Fixture-only on-call evidence for deterministic smoke validation.",
...
def main() -> None:
    ...
    require(
        default_summary["on_call_rotation_available_for_review"] is False,
        "default on-call must not be available",
    )
    require(
    ...
    require(
        complete_summary["on_call_rotation_available_for_review"] is True,
        "complete fixture on-call available",
    )
    require(
    ...
    require(
        ...
        "complete readiness on-call available",
    )
    ...
    require(
        ...
        "unsafe on-call remains unavailable",
    )
    ...
    for path in [
        "/phase_b_product/commercial_readiness/ON_CALL_EVIDENCE_BUILDER_V0_1.md",
        "/docs/strategy/SAEE_ON_CALL_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md",
        "/phase_b_product/commercial_readiness/support_evidence/on_call_evidence_input.template.json",
        "/phase_b_product/commercial_readiness/support_evidence/on_call_evidence_builder_output.local.json",
        "/phase_b_product/commercial_readiness/support_evidence/production_support_sla_evidence.from_on_call.local.json",
        "/phase_b_product/commercial_readiness/support_evidence/on_call_evidence_builder_report.md",
        "/scripts/saee_on_call_evidence_builder.py",
        "/scripts/saee_on_call_evidence_builder_smoke.py",
    ]:
        require(path in llms, "llms.txt missing path: " + path)

    index = json.loads((ROOT / "agent-index.json").read_text(encoding="utf-8"))
    entry = index.get("on_call_evidence_builder_v0_1", {})
    expected = {
        "status": "local_builder_available_default_hold",
        "builder_scope": "human_filled_on_call_rotation_to_production_support_evidence",
        "on_call_rotation_available_for_review": False,
        "production_support_available": False,
    ...
    print(
        "SAEE_ON_CALL_EVIDENCE_BUILDER_SMOKE: PASS "
        "default_hold=true complete_fixture_on_call_available=true "
