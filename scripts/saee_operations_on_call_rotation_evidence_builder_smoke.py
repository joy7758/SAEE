#!/usr/bin/env python3
"""Smoke check for the SAEE operations-on-call-rotation evidence builder."""
...
BUILDER_SCRIPT = ROOT / "scripts/saee_operations_on_call_rotation_evidence_builder.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_OPERATIONS_ON_CALL_ROTATION_EVIDENCE_BUILDER_SMOKE: FAIL: " + message)
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    source_notes = {
        key: f"Human-reviewed operations-on-call-rotation source note for {key}."
    ...
    return {
        ...
        "on_call_rotation_owner": "Fixture On-call Rotation Owner",
        "operations_reviewer_name": "Fixture Operations Reviewer",
        "decision_summary": "Fixture-only on-call rotation evidence for deterministic smoke validation.",
...
def main() -> None:
    ...
    require(
        default_summary["operations_on_call_rotation_available_for_review"] is False,
        "default external on-call rotation must not be available",
    )
    require(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        complete_input_path = tmp / "complete_on_call_rotation_input.json"
        complete_output_path = tmp / "complete_builder_output.json"
        complete_operations_path = tmp / "complete_operations_evidence.json"
        unsafe_input_path = tmp / "unsafe_on_call_rotation_input.json"
    ...
    require(
        complete_summary["operations_on_call_rotation_available_for_review"] is True,
        "complete fixture production external on-call rotation available",
    )
    require(
    ...
    require(
        complete_readiness["on_call_rotation_available"] is True,
        "complete readiness production external on-call rotation available",
    )
    ...
    require(
        complete_readiness["external_alert_delivery_available"] is False,
        "external alert delivery false",
    )
    require(
    ...
    require(
        unsafe_readiness["on_call_rotation_available"] is False,
        "unsafe external on-call rotation remains unavailable",
    )
    ...
    for path in [
        "/phase_b_product/commercial_readiness/OPERATIONS_ON_CALL_ROTATION_EVIDENCE_BUILDER_V0_1.md",
        "/docs/strategy/SAEE_OPERATIONS_ON_CALL_ROTATION_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md",
        "/phase_b_product/commercial_readiness/operations_evidence/operations_on_call_rotation_evidence_input.template.json",
        "/phase_b_product/commercial_readiness/operations_evidence/operations_on_call_rotation_evidence_builder_output.local.json",
        "/phase_b_product/commercial_readiness/operations_evidence/production_operations_evidence.from_operations_on_call_rotation.local.json",
        "/phase_b_product/commercial_readiness/operations_evidence/operations_on_call_rotation_evidence_builder_report.md",
        "/scripts/saee_operations_on_call_rotation_evidence_builder.py",
        "/scripts/saee_operations_on_call_rotation_evidence_builder_smoke.py",
    ]:
        require(path in llms, "llms.txt missing path: " + path)

    index = json.loads((ROOT / "agent-index.json").read_text(encoding="utf-8"))
    entry = index.get("operations_on_call_rotation_evidence_builder_v0_1", {})
    expected = {
        "status": "local_builder_available_default_hold",
        "builder_scope": "human_filled_operations_on_call_rotation_to_production_operations_evidence",
        "operations_on_call_rotation_available_for_review": False,
        "production_operations_ready": False,
        "operations_on_call_rotation_available": False,
    ...
    print(
        "SAEE_OPERATIONS_ON_CALL_ROTATION_EVIDENCE_BUILDER_SMOKE: PASS "
        "default_hold=true complete_fixture_operations_on_call_rotation_available=true "
