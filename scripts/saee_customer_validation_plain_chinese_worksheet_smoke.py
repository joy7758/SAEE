#!/usr/bin/env python3
"""Smoke test the plain-Chinese customer validation worksheet."""
...
OUT = ROOT / "phase_b_product/commercial_readiness/customer_validation_evidence/customer_validation_plain_chinese_worksheet"
...
def fail(message: str) -> None:
    raise SystemExit("SAEE_CUSTOMER_VALIDATION_PLAIN_CHINESE_WORKSHEET_SMOKE: FAIL " + message)
...
def main() -> None:
    ...
    expected = {
        ...
        "question_count": 18,
        "boundary_confirmation_count": 5,
        "evidence_review_count": 26,
    ...
    require(not ANSWER_INPUT.exists(), "plain Chinese worksheet must not create human-filled answers")
    require(not TARGET_ENTRY.exists(), "plain Chinese worksheet must not write final session entry")
    ...
    for token in [
        ...
        "Copy these lines into customer_validation_answers.human_filled.md",
    ]:
    ...
    entry = agent_index.get("customer_validation_plain_chinese_worksheet_v0_1")
    require(isinstance(entry, dict), "agent-index missing customer_validation_plain_chinese_worksheet_v0_1")
    for key in [
        "status",
    ...
    print(
        "SAEE_CUSTOMER_VALIDATION_PLAIN_CHINESE_WORKSHEET_SMOKE: PASS "
        "questions=18 customer_validated=false"
    )
