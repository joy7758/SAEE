#!/usr/bin/env python3
"""Build refund-policy evidence from human-filled review input.

This builder converts local, human-filled refund-policy evidence into the
production billing/revenue evidence shape consumed by commercial readiness
checks. It does not publish a refund policy, approve cancellation handling,
...
DEFAULT_INPUT_PATH = OUTPUT_DIR / "refund_policy_evidence_input.template.json"
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "refund_policy_evidence_builder_output.local.json"
DEFAULT_BILLING_REVENUE_OUTPUT_PATH = (
    OUTPUT_DIR / "production_billing_revenue_evidence.from_refund_policy.local.json"
)
REPORT_PATH = OUTPUT_DIR / "refund_policy_evidence_builder_report.md"
DOC_PATH = ROOT / "phase_b_product/commercial_readiness/REFUND_POLICY_EVIDENCE_BUILDER_V0_1.md"
GATE_PATH = ROOT / "docs/strategy/SAEE_REFUND_POLICY_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md"
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_REFUND_POLICY_EVIDENCE_BUILDER: FAIL: " + message)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_REFUND_POLICY_EVIDENCE_BUILDER: FAIL: invalid input {path}: {exc}"
        ) from exc
...
def build_billing_revenue_evidence(
    ...
    *,
    ...
    evidence: dict[str, Any] = {
        ...
        "evidence_scope": "human_filled_refund_policy_to_production_billing_revenue_evidence",
        "evidence_version": "v0.1",
        "generated_by": "scripts/saee_refund_policy_evidence_builder.py",
...
def build_from_input(
    ...
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "refund_policy_evidence_complete_for_review": readiness_result[
        ...
        "next_action": (
            ...
            "and tenant-boundary owners may fill the refund-policy input with "
            "source-backed refund eligibility, cancellation, trial conversion, "
            "service-failure remedy, and support escalation references. The "
...
def report_markdown(summary: dict[str, Any]) -> str:
    return f"""# SAEE Refund Policy Evidence Builder Report
...
- refund_policy_evidence_complete_for_review: {str(summary['refund_policy_evidence_complete_for_review']).lower()}
...
def write_docs(summary: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Refund Policy Evidence Builder v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        return
    print(
        "SAEE_REFUND_POLICY_EVIDENCE_BUILDER: PASS "
