#!/usr/bin/env python3
"""Smoke check for the OAuth/OIDC approval input validator."""
...
VALIDATOR_SCRIPT = ROOT / "scripts/saee_oauth_oidc_approval_input_validator.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_OAUTH_OIDC_APPROVAL_INPUT_VALIDATOR_SMOKE: FAIL: " + message
        )
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    data["evidence_source_notes"] = "Fixture-only OAuth/OIDC evidence review note."
...
def main() -> None:
    ...
    entry = index.get("oauth_oidc_approval_input_validator_v0_1", {})
    for key, value in {
        "status": "hold",
        "validator_type": "saee_oauth_oidc_approval_input_validator",
        "builder_ready": False,
    ...
    print(
        "SAEE_OAUTH_OIDC_APPROVAL_INPUT_VALIDATOR_SMOKE: PASS "
