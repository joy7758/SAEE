#!/usr/bin/env python3
"""Build SLA evidence from a human-filled support SLA input.

This builder converts local, human-filled SLA approval evidence into the
production support/SLA evidence shape consumed by commercial readiness checks.
It does not publish SLA terms, contact customers or vendors, approve legal
terms by itself, start support operations, close blockers, modify product
...
DEFAULT_INPUT_PATH = OUTPUT_DIR / "sla_evidence_input.template.json"
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "sla_evidence_builder_output.local.json"
DEFAULT_SUPPORT_OUTPUT_PATH = OUTPUT_DIR / "production_support_sla_evidence.from_sla.local.json"
REPORT_PATH = OUTPUT_DIR / "sla_evidence_builder_report.md"
DOC_PATH = ROOT / "phase_b_product/commercial_readiness/SLA_EVIDENCE_BUILDER_V0_1.md"
GATE_PATH = ROOT / "docs/strategy/SAEE_SLA_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md"
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_SLA_EVIDENCE_BUILDER: FAIL: " + message)
...
def default_input_template() -> dict[str, Any]:
    return {
        ...
        "legal_reviewer_name": "",
        "decision_summary": "",
        "evidence_review": {key: False for key in SLA_KEYS},
        "source_notes_by_key": {key: "" for key in SLA_KEYS},
        "sla_evidence_slots": [
            {
                "evidence_key": key,
                "evidence_reference": "",
                "owner_named": False,
                "legal_or_commercial_reviewed": False,
                "human_source_note": "",
            }
            for key in SLA_KEYS
        ],
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_SLA_EVIDENCE_BUILDER: FAIL: invalid input {path}: {exc}"
        ) from exc
...
def evidence_review_flags(data: dict[str, Any]) -> dict[str, bool]:
    ...
    if not isinstance(review, dict):
        review = {}
    return {key: bool_value(review, key) for key in SLA_KEYS}
...
def source_notes(data: dict[str, Any]) -> dict[str, str]:
    ...
    if not isinstance(notes, dict):
        return {}
    return {key: str(notes.get(key, "")).strip() for key in SLA_KEYS}
...
def completed_sla_slots(data: dict[str, Any]) -> dict[str, dict[str, Any]]:
    slots = data.get("sla_evidence_slots", [])
    ...
    for slot in slots:
        ...
        if key not in SLA_KEYS:
            continue
        if (
            str(slot.get("evidence_reference", "")).strip()
            and slot.get("owner_named") is True
            and slot.get("legal_or_commercial_reviewed") is True
...
def complete_input(data: dict[str, Any]) -> bool:
    ...
    return (
        ...
        and all(bool(notes.get(key)) for key in SLA_KEYS)
        and all(key in complete_slots for key in SLA_KEYS)
...
def build_support_evidence(
    ...
    *,
    ...
    evidence: dict[str, Any] = {
        ...
        "evidence_scope": "human_filled_sla_approval_to_production_support_evidence",
        "evidence_version": "v0.1",
        "generated_by": "scripts/saee_sla_evidence_builder.py",
        ...
        "sla_terms_owner_recorded": bool(str(data.get("sla_terms_owner", "")).strip()),
        "legal_reviewer_name_recorded": bool(str(data.get("legal_reviewer_name", "")).strip()),
        "completed_sla_slot_count": len(complete_slots),
        "sla_published_by_codex": False,
        "sla_approved_by_codex": False,
        "legal_review_completed_by_codex": False,
        "support_hours_published_by_codex": False,
    ...
    for key in SUPPORT_CONTACT_KEYS + CUSTOMER_SUPPORT_KEYS:
        evidence[key] = False
    for key in SLA_KEYS:
...
def build_from_input(
    ...
    *,
    ...
    flags = evidence_review_flags(data)
    missing = [key for key in SLA_KEYS if not flags[key]]
    missing_slots = [key for key in SLA_KEYS if key not in completed_sla_slots(data)]
    ...
    summary: dict[str, Any] = {
        ...
        "sla_available_for_review": readiness["sla_available"],
        ...
        "codex_inferred_missing_evidence": False,
        "next_action": (
            "If a human fills the SLA evidence input with source notes, the "
            "generated support evidence can be used as one input to support "
            "readiness review. Support contact, customer support, and on-call "
...
def report_markdown(summary: dict[str, Any]) -> str:
    return f"""# SAEE SLA Evidence Builder Report
...
- sla_available_for_review: {str(summary['sla_available_for_review']).lower()}
...
def write_docs(summary: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE SLA Evidence Builder v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_SLA_EVIDENCE_BUILDER: PASS "
            f"path={args.output} status={summary['status']} "
            f"sla_available_for_review={str(summary['sla_available_for_review']).lower()} "
