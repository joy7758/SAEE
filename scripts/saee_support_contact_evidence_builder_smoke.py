#!/usr/bin/env python3
"""Smoke check for the SAEE support-contact evidence builder."""
...
BUILDER_SCRIPT = ROOT / "scripts/saee_support_contact_evidence_builder.py"
INPUT_TEMPLATE = (
    ROOT
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_SUPPORT_CONTACT_EVIDENCE_BUILDER_SMOKE: FAIL: " + message
        )
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    source_notes = {
        key: f"Human-reviewed support contact source note for {key}."
    ...
    return {
        ...
        "decision_summary": "Fixture-only support contact evidence for deterministic smoke validation.",
...
def main() -> None:
    ...
    require(INPUT_TEMPLATE.exists(), "support contact decision input template missing")
    ...
    require(
        default_summary["support_contact_available_for_review"] is False,
        "default support contact must not be available",
    )
    require(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        tmp = Path(tmpdir)
        complete_input_path = tmp / "complete_support_contact_input.json"
        complete_output_path = tmp / "complete_builder_output.json"
        complete_support_path = tmp / "complete_support_evidence.json"
        unsafe_input_path = tmp / "unsafe_support_contact_input.json"
        ...
        complete_readiness = support_readiness(
            complete_support_path,
            "support-fixture@example.invalid",
        )
        unsafe_readiness = support_readiness(
            unsafe_support_path,
            "support-fixture@example.invalid",
        )
    ...
    require(
        complete_summary["support_contact_available_for_review"] is True,
        "complete fixture support contact available",
    )
    require(
    ...
    require(
        complete_readiness["support_contact_available"] is True,
        "complete readiness support contact available",
    )
    ...
    require(
        unsafe_readiness["support_contact_available"] is False,
        "unsafe support contact remains unavailable",
    )
    ...
    for path in [
        "/phase_b_product/commercial_readiness/SUPPORT_CONTACT_EVIDENCE_BUILDER_V0_1.md",
        "/docs/strategy/SAEE_SUPPORT_CONTACT_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md",
        "/phase_b_product/commercial_readiness/support_evidence/support_contact_evidence_builder_output.local.json",
        "/phase_b_product/commercial_readiness/support_evidence/production_support_sla_evidence.from_support_contact.local.json",
        "/phase_b_product/commercial_readiness/support_evidence/support_contact_evidence_builder_report.md",
        "/scripts/saee_support_contact_evidence_builder.py",
        "/scripts/saee_support_contact_evidence_builder_smoke.py",
    ]:
        require(path in llms, "llms.txt missing path: " + path)

    index = json.loads((ROOT / "agent-index.json").read_text(encoding="utf-8"))
    entry = index.get("support_contact_evidence_builder_v0_1", {})
    expected = {
    ...
    print(
        "SAEE_SUPPORT_CONTACT_EVIDENCE_BUILDER_SMOKE: PASS "
        "default_hold=true complete_fixture_support_contact_available=true "
