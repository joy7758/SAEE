#!/usr/bin/env python3
"""Build tax-review evidence from human-filled review input.

This builder converts local, human-filled tax-review evidence into the
production billing/revenue evidence shape consumed by commercial readiness
checks. It does not contact tax advisors or legal counsel, complete tax
...
DEFAULT_INPUT_PATH = OUTPUT_DIR / "tax_review_evidence_input.template.json"
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "tax_review_evidence_builder_output.local.json"
DEFAULT_BILLING_REVENUE_OUTPUT_PATH = (
    OUTPUT_DIR / "production_billing_revenue_evidence.from_tax_review.local.json"
)
REPORT_PATH = OUTPUT_DIR / "tax_review_evidence_builder_report.md"
DOC_PATH = ROOT / "phase_b_product/commercial_readiness/TAX_REVIEW_EVIDENCE_BUILDER_V0_1.md"
GATE_PATH = ROOT / "docs/strategy/SAEE_TAX_REVIEW_EVIDENCE_BUILDER_RECOMMENDATION_GATE.md"
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_TAX_REVIEW_EVIDENCE_BUILDER: FAIL: " + message)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_TAX_REVIEW_EVIDENCE_BUILDER: FAIL: invalid input {path}: {exc}"
        ) from exc
...
def complete_input(data: dict[str, Any]) -> bool:
    ...
    return (
        data.get("template_type") == "saee_tax_review_evidence_input"
...
def build_billing_revenue_evidence(
    ...
    *,
    ...
    evidence: dict[str, Any] = {
        ...
        "evidence_scope": "human_filled_tax_review_to_production_billing_revenue_evidence",
        "evidence_version": "v0.1",
        "generated_by": "scripts/saee_tax_review_evidence_builder.py",
        ...
        "tax_owner_recorded": bool(str(data.get("tax_owner", "")).strip()),
...
def build_from_input(
    ...
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "tax_review_evidence_complete_for_review": readiness_result[
        ...
        "codex_inferred_missing_evidence": False,
        "next_action": (
            "Human legal, tax, accounting, commercial, and billing owners may "
            "fill the tax-review input with source-backed jurisdiction, tax "
            "obligation, invoice wording, currency policy, and tax collection "
            "approval references. The generated evidence remains one input to "
            "a later go/no-go profile."
        ),
    }
...
def report_markdown(summary: dict[str, Any]) -> str:
    return f"""# SAEE Tax Review Evidence Builder Report
...
- tax_review_evidence_complete_for_review: {str(summary['tax_review_evidence_complete_for_review']).lower()}
...
def write_docs(summary: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Tax Review Evidence Builder v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        return
    print(
        "SAEE_TAX_REVIEW_EVIDENCE_BUILDER: PASS "
