#!/usr/bin/env python3
"""Validate human-filled formal-security-review input before the evidence builder.

This validator checks completeness and boundary safety only. It does not
perform a security review, approve a report, contact reviewers or vendors, run
...
DEFAULT_OUTPUT_PATH = (
    OUTPUT_DIR / "formal_security_review_approval_input_validation.local.json"
)
REPORT_PATH = OUTPUT_DIR / "formal_security_review_approval_input_validation.md"
DOC_PATH = (
    ROOT
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_FORMAL_SECURITY_REVIEW_APPROVAL_INPUT_VALIDATOR: FAIL " + message
        )
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            "SAEE_FORMAL_SECURITY_REVIEW_APPROVAL_INPUT_VALIDATOR: FAIL "
...
def missing_evidence_review(data: dict[str, Any]) -> list[str]:
    ...
    return [key for key in FORMAL_SECURITY_REVIEW_KEYS if not flags[key]]


def missing_source_notes(data: dict[str, Any]) -> list[str]:
    notes = source_notes(data)
    return [key for key in FORMAL_SECURITY_REVIEW_KEYS if not notes.get(key)]


def missing_review_artifacts(data: dict[str, Any]) -> list[str]:
    artifacts = completed_artifacts(data)
    return [key for key in FORMAL_SECURITY_REVIEW_KEYS if key not in artifacts]
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_by": "scripts/saee_formal_security_review_approval_input_validator.py",
        ...
        "required_formal_security_review_evidence_item_count": len(
            FORMAL_SECURITY_REVIEW_KEYS
        ),
        ...
        "next_action": (
            "If validation_status is pass, a human may run the formal security "
            "review evidence builder in a separate approved evidence request; "
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE Formal Security Review Approval Input Validation
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Formal Security Review Approval Input Validator v0.1
...
target_blocker_id: formal_security_review
required_formal_security_review_evidence_item_count: {validation['required_formal_security_review_evidence_item_count']}
blockers_closed_by_validator: 0
formal_security_review_approved_by_validator: false
formal_security_review_completed_by_validator: false
formal_security_review_report_approved_by_validator: false
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_FORMAL_SECURITY_REVIEW_APPROVAL_INPUT_VALIDATOR: PASS "
