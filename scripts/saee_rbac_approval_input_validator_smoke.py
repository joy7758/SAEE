#!/usr/bin/env python3
"""Smoke check for the RBAC approval input validator."""
...
VALIDATOR_SCRIPT = ROOT / "scripts/saee_rbac_approval_input_validator.py"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_RBAC_APPROVAL_INPUT_VALIDATOR_SMOKE: FAIL: " + message)
...
def complete_input(*, unsafe: bool = False) -> dict[str, object]:
    ...
    data["human_reviewer_name"] = "Human Security Owner"
    data["review_date"] = "2026-07-05"
    data["evidence_source_notes"] = "Fixture-only RBAC evidence review note."
...
def main() -> None:
    ...
    require(
        complete_summary["rbac_enforced_in_production"] is False,
        "complete input does not enforce production RBAC",
    )
    ...
    entry = index.get("rbac_approval_input_validator_v0_1", {})
    for key, value in {
        "status": "hold",
        "validator_type": "saee_rbac_approval_input_validator",
        "builder_ready": False,
    ...
    print(
        "SAEE_RBAC_APPROVAL_INPUT_VALIDATOR_SMOKE: PASS "
