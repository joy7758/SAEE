...
def main() -> None:
    ...
    require(plan["external_state"]["alibaba_product_partner_application"] == "submitted_under_review", "Alibaba state")
    alibaba = next(item for item in multi_cloud["providers"] if item["provider"] == "Alibaba Cloud")
    require(alibaba["current_state"] == "formal_product_partner_application_submitted_under_review", "Alibaba receipt matrix")
    ...
    require("answer: conditional" in gate and "status: open_external_qoder_application_submitted_waiting_review" in gate, "recommendation gate")
