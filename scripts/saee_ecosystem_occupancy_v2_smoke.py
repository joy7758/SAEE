...
def main() -> None:
    ...
    qoder_install_preflight = load("agent-interface/ecosystem/saee-qoder-cli-installation-preflight.v1.json")
    ...
    baidu_consultation = load("agent-interface/ecosystem/saee-baidu-partner-consultation-submission-receipt.v1.json")
    baidu_qianfan_ticket = load("agent-interface/ecosystem/saee-baidu-qianfan-agent-technical-ticket-submission-receipt.v1.json")
    ...
    require(qoder_install_preflight["truth_boundary"]["qoder_cli_installation_preflight_ready"] is True, "Qoder CLI installation preflight")
    require(qoder_install_preflight["authorization_gate"]["installation_authorized"] is False, "Qoder CLI install authorization")
    require(qoder_install_preflight["authorization_gate"]["installer_executed"] is False, "Qoder CLI installer execution boundary")
    require(qoder_install_preflight["bounded_validation_policy"]["external_world_actions_allowed"] is False, "Qoder CLI validation boundary")
    ...
    require(alibaba_qoder_support["truth_boundary"]["provider_scoping_question_received"] is True, "Alibaba Qoder scoping question")
    require(alibaba_qoder_support["truth_boundary"]["qoder_cn_scope_confirmed"] is True, "Alibaba Qoder CN scope")
    ...
    require(baidu_consultation["submission_event_count"] == 2, "Baidu consultation submission events")
    require(baidu_consultation["truth_boundary"]["duplicate_submission_does_not_count_as_second_vendor_or_completed_conversation"] is True, "Baidu duplicate submission KPI boundary")
    require(plan["external_state"]["baidu_partner_consultation_submission_event_count"] == 2, "Baidu consultation plan linkage")
    require(baidu_qianfan_ticket["ticket_id"] == "20260713170942316", "Baidu Qianfan ticket ID")
    require(baidu_qianfan_ticket["related_product"] == "百度千帆Agent开发平台", "Baidu Qianfan ticket product")
    require(baidu_qianfan_ticket["truth_boundary"]["engineer_acknowledgement_received"] is True, "Baidu Qianfan ticket acknowledgement")
    require(baidu_qianfan_ticket["truth_boundary"]["technical_conversation_in_progress"] is True, "Baidu Qianfan conversation in progress")
    require(baidu_qianfan_ticket["truth_boundary"]["technical_conversation_completed"] is False, "Baidu Qianfan conversation boundary")
    require(plan["kpis"]["ecosystem"]["technical_conversation_request_events_submitted"] == 5, "technical request count")
    require(plan["kpis"]["ecosystem"]["technical_conversation_distinct_vendor_targets"] == 2, "technical target count")
    require(plan["kpis"]["ecosystem"]["cloud_vendor_technical_conversation_in_progress"] == 1, "technical conversation in-progress count")
    require(plan["kpis"]["ecosystem"]["cloud_vendor_business_handoff_pending"] == 1, "business handoff pending count")
    ...
    require("answer: conditional" in gate and "status: open_external_qianfan_technical_ticket_in_progress_qoder_business_handoff_pending" in gate, "recommendation gate")
