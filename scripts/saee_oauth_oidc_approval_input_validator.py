#!/usr/bin/env python3
"""Validate human-filled OAuth/OIDC evidence input before builder use.
...
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "oauth_oidc_approval_input_validation.local.json"
REPORT_PATH = OUTPUT_DIR / "oauth_oidc_approval_input_validation.md"
DOC_PATH = (
    ROOT / "phase_b_product/commercial_readiness/OAUTH_OIDC_APPROVAL_INPUT_VALIDATOR_V0_1.md"
)
GATE_PATH = (
    ROOT
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_OAUTH_OIDC_APPROVAL_INPUT_VALIDATOR: FAIL " + message)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"SAEE_OAUTH_OIDC_APPROVAL_INPUT_VALIDATOR: FAIL invalid input {path}: {exc}"
        ) from exc
...
def evidence_review_flags(data: dict[str, Any]) -> dict[str, bool]:
    ...
    if not isinstance(review, dict):
        review = {}
    return {key: review.get(key) is True for key in OAUTH_OIDC_KEYS}


def missing_evidence_review(data: dict[str, Any]) -> list[str]:
    flags = evidence_review_flags(data)
    return [key for key in OAUTH_OIDC_KEYS if not flags[key]]
...
def missing_source_notes(data: dict[str, Any]) -> list[str]:
    ...
    if not isinstance(notes, dict):
        notes = {}
    return [key for key in OAUTH_OIDC_KEYS if not nonempty_text(notes.get(key))]
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "target_evidence_keys": list(OAUTH_OIDC_KEYS),
        "generated_by": "scripts/saee_oauth_oidc_approval_input_validator.py",
        ...
        "required_review_key_count": len(OAUTH_OIDC_KEYS),
        "completed_review_key_count": len(OAUTH_OIDC_KEYS) - len(missing_review),
        ...
        "next_action": (
            ...
            "request; otherwise complete missing OAuth/OIDC source notes or "
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE OAuth/OIDC Approval Input Validation
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE OAuth/OIDC Approval Input Validator v0.1
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_OAUTH_OIDC_APPROVAL_INPUT_VALIDATOR: PASS "
