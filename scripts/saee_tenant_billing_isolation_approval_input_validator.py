#!/usr/bin/env python3
"""Validate human-filled tenant-billing-isolation input before the builder.

This validator checks completeness and boundary safety only. It does not
approve tenant billing isolation, test cross-tenant billing access, configure
payment-provider tenant mapping, collect payment, validate revenue, close
blockers, modify product behavior, or claim production readiness.
"""
...
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "tenant_billing_isolation_approval_input_validation.local.json"
REPORT_PATH = OUTPUT_DIR / "tenant_billing_isolation_approval_input_validation.md"
DOC_PATH = (
    ROOT
    / "phase_b_product/commercial_readiness/"
    "TENANT_BILLING_ISOLATION_APPROVAL_INPUT_VALIDATOR_V0_1.md"
)
GATE_PATH = (
    ROOT
    / "docs/strategy/"
    "SAEE_TENANT_BILLING_ISOLATION_APPROVAL_INPUT_VALIDATOR_RECOMMENDATION_GATE.md"
)
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit(
            "SAEE_TENANT_BILLING_ISOLATION_APPROVAL_INPUT_VALIDATOR: FAIL "
            + message
        )
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_by": "scripts/saee_tenant_billing_isolation_approval_input_validator.py",
        ...
        "required_tenant_billing_isolation_evidence_item_count": len(TARGET_KEYS),
        ...
        "next_action": (
            "If validation_status is pass, a human may run the tenant billing "
            "isolation evidence builder in a separate approved evidence request; "
            "otherwise complete missing input fields or resolve boundary "
            "violations first."
        ),
    }
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE Tenant Billing Isolation Approval Input Validation
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Tenant Billing Isolation Approval Input Validator v0.1
...
target_blocker_id: tenant_billing_isolation
required_tenant_billing_isolation_evidence_item_count: {validation['required_tenant_billing_isolation_evidence_item_count']}
blockers_closed_by_validator: 0
tenant_billing_isolation_approved_by_validator: false
tenant_billing_isolation_published_by_validator: false
tenant_billing_isolation_completed_by_validator: false
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_TENANT_BILLING_ISOLATION_APPROVAL_INPUT_VALIDATOR: PASS "
