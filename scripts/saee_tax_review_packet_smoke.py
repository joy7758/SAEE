#!/usr/bin/env python3
"""Smoke check for the SAEE tax review packet."""
...
GATE_PATH = ROOT / "docs/strategy/SAEE_TAX_REVIEW_PACKET_RECOMMENDATION_GATE.md"


def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_TAX_REVIEW_PACKET_SMOKE: FAIL: " + message)
...
def main() -> None:
    ...
    require(
        required_sections <= set(packet.get("required_tax_review_sections", [])),
        "missing required tax review sections",
    )
    ...
    for path in [
        ...
        "/docs/strategy/SAEE_TAX_REVIEW_PACKET_RECOMMENDATION_GATE.md",
        "/scripts/saee_tax_review_packet.py",
        "/scripts/saee_tax_review_packet_smoke.py",
    ]:
        require(path in llms, f"llms.txt missing {path}")

    index = json.loads((ROOT / "agent-index.json").read_text(encoding="utf-8"))
    entry = index.get("tax_review_packet_v0_1", {})
    expected_entry = {
    ...
    print(
        "SAEE_TAX_REVIEW_PACKET_SMOKE: PASS "
        "ready_for_human_review=true tax_review_evidence_complete=false "
