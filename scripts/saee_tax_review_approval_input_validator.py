#!/usr/bin/env python3
"""Validate human-filled tax-review input before the evidence builder.

This validator checks completeness and boundary safety only. It does not
complete tax review, contact tax advisors or legal counsel, configure tax
rates, start tax collection, publish invoice wording, publish currency policy,
collect payment, validate revenue, close blockers, or claim production
readiness.
"""
...
from scripts.saee_tax_review_evidence_builder import (
    DEFAULT_INPUT_PATH,
...
DEFAULT_OUTPUT_PATH = OUTPUT_DIR / "tax_review_approval_input_validation.local.json"
REPORT_PATH = OUTPUT_DIR / "tax_review_approval_input_validation.md"
DOC_PATH = (
    ROOT
...
def require(condition: bool, message: str) -> None:
    if not condition:
        raise SystemExit("SAEE_TAX_REVIEW_APPROVAL_INPUT_VALIDATOR: FAIL " + message)
...
def read_json(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            "SAEE_TAX_REVIEW_APPROVAL_INPUT_VALIDATOR: FAIL "
...
def build_validation(input_path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "generated_by": "scripts/saee_tax_review_approval_input_validator.py",
        ...
        "required_tax_review_evidence_item_count": len(TARGET_KEYS),
        ...
        "next_action": (
            "If validation_status is pass, a human may run the tax review "
...
def report_markdown(validation: dict[str, Any]) -> str:
    return f"""# SAEE Tax Review Approval Input Validation
...
def write_docs(validation: dict[str, Any]) -> None:
    ...
    DOC_PATH.write_text(
        f"""# SAEE Tax Review Approval Input Validator v0.1
...
target_blocker_id: tax_review
required_tax_review_evidence_item_count: {validation['required_tax_review_evidence_item_count']}
blockers_closed_by_validator: 0
tax_review_approved_by_validator: false
tax_review_completed_by_validator: false
tax_rate_configured_by_validator: false
...
def main() -> None:
    ...
    if args.json:
        ...
        print(
            "SAEE_TAX_REVIEW_APPROVAL_INPUT_VALIDATOR: PASS "
