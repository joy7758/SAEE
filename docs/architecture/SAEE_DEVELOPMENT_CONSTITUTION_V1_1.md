...
constitution_patch_version=1.1.2
last_amended_date=2026-07-19
...
publication_venue_policy=real_peer_reviewed_journal_zero_mandatory_author_cost
...
## 第十一章：学术期刊真实性与零费用发表门禁

### 第二十二条：期刊路线必须是真实学术期刊

任何新的期刊选择、转投、改投、重投或投稿门户录入，必须先证明目标是可核验的
peer-reviewed scholarly journal（同行评审学术期刊），至少具有：

1. 出版商或学会的官方期刊页面；
2. 可核验的 ISSN 或 eISSN；
3. 明确的同行评审政策、编辑委员会和出版伦理说明；
4. 可持续访问的卷期或文章归档记录；
5. 独立于 conference、workshop、poster、Late-Breaking Abstract、magazine、blog、
   preprint repository 或单纯活动页面的期刊身份。

以上证据缺少任一关键项，或者出版主体、费用、同行评审与归档信息无法从官方来源
核验时，必须输出 `VENUE_NOT_ELIGIBLE` 并停止该路线。不得把会议录用、海报展示、
摘要上线、预印本存储或 DOI 仓储记录写成期刊发表。

### 第二十三条：作者强制发表费用必须为零

SAEE 论文只能选择作者侧 mandatory publication cost（强制发表费用）为 `0` 的路线。
允许的路线仅包括：

- subscription / traditional publishing（订阅制 / 传统发表），且作者无需支付 APC、
  submission fee、page charge、mandatory color charge 或其他强制费用；
- diamond open access（钻石开放获取），且作者无需支付任何强制费用。

期刊提供可选付费 OA 不构成阻塞，前提是官方规则明确允许作者选择免费的传统发表
路线。fully open access 且 mandatory APC、必须缴纳投稿费或版面费、依赖未提前批准的
waiver、以及要求缴纳会议注册费才能发表或展示的路线均不合格。

```text
mandatory_author_cost_limit=0
unknown_cost_policy=stop_and_reject_venue
waiver_dependent_route_without_preapproval=not_eligible
workflow_override_allowed=false
```

### 第二十四条：费用核验必须前置并可追溯

在生成投稿终稿、进入门户或上传文件前，Agent 必须记录：

1. 官方 author guidelines；
2. 官方 fee / open-access 页面；
3. 核验日期；
4. 所选择的免费发表路线；
5. `mandatory_author_cost=0` 的明确结论。

费用未知、页面冲突、录用后才能获知费用、或只有申请减免后才可能免费，均视为
`COST_GATE_FAILED`。package ready、portal draft、upload、proof 和 final submission 必须与
费用门禁分别记录，不得用材料就绪掩盖费用不合格。

### 第二十五条：ALIFE 2026 路线的适用决定

ALIFE 2026 `lb120` 是 Late-Breaking Abstract / poster conference route，不是真实期刊
发表路线。其 `Accept (Confirmed)` 只证明会议摘要录用和作者曾确认接受，不证明期刊
接收、proceedings 收录、DOI 或期刊发表。

作者于 2026-07-19 决定不支付会议注册费、不完成会议注册、不继续该展示路线。该
决定冻结 ALIFE 2026 注册与付款动作，但不伪造外部门户撤回：在出现 Linklings 或主办方
撤回证据前，必须同时保留 `external_portal_status=Accept (Confirmed)` 与
