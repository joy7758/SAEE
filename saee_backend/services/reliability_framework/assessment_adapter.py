...
def _get_task_status(completed: bool) -> str:
    return "OBSERVED_PASS" if completed else "NOT_ASSESSED"


def _get_recovery_status(run: dict[str, Any], completed: bool) -> str:
...
def _get_boundary_status(run: dict[str, Any], completed: bool) -> str:
    if "boundary_preserved" not in run:
        return "NOT_ASSESSED"
    if not completed:
        return "NOT_ASSESSED"
...
def _get_evidence_status(evidence_result: str) -> str:
...
def assess_reliability_run(
    ...
    if source_type == "RECOMMENDATION_BENCHMARK_RUN":
        ...
        task_status = _get_task_status(completed)
        recovery_status = _get_recovery_status(run, completed)
        boundary_status = _get_boundary_status(run, completed)
        evidence_status = _get_evidence_status(evidence_result)
